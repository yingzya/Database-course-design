            ...
            <template #default="scope">
              <!-- <el-input v-model="scope.row.name" placeholder="请输入口味名称" /> -->
               ...
               @change="changeFlavorName(scope.row)">
                <el-option
                  v-for="dishFlavor in dishFlavourListSelect"
                  :key="dishFlavor.name" 
...
//------------------------------
//定义口味名称和口味列表的静态数据
const dishFlavourListSelect = ref([
...
//定义改变改变口味名称时更新当前选中的口味列表
function changeFlavorName(row){
  //清空当前行value
  row.value = [];
  //根据选中的name去查找静态数据的value
  checkValueList.value = dishFlavourListSelect.value.find(item => item.name === row.name).value;
}
